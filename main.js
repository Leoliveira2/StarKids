...
// Simples renderização inicial
function render() {
  const container = document.getElementById('app');
  if (!container) return;
  container.innerHTML = appState.showLogin ? renderLoginScreen() : renderDashboard();
}
...
function renderDashboard() {
  return `<div class="text-white text-center">
    <h2 class="text-2xl font-bold">Bem-vindo(a), ${appState.currentUser.name}!</h2>
    <p class="mt-2">Mascote: ${mascots[appState.currentUser.mascot].emoji}</p>
    <button onclick="logout()" class="mt-4 bg-red-500 px-4 py-2 rounded">Sair</button>
  </div>`;
}
